...
/** Mostrado quando o usuario nao preencheu o campo. */
const TRACO = "—";

// ------------------------------------------------------------
// Tipo auxiliar para colunas
// ------------------------------------------------------------
//
// A versao de @types/pdfmake usada pelo projeto possui uma
// tipagem muito restritiva para Content. Em tempo de execucao,
// pdfmake aceita normalmente objetos com width + stack dentro
// de columns.
//
// Este tipo representa exatamente esse formato.
// ------------------------------------------------------------
 ...
 * Campo vazio, texto invalido ou negativo caem no padrao.
 */
...
/** Faixa preta com titulo branco. */
function faixa(
    texto: string,
    margem: [number, number, number, number],
): Content {
    return {
        table: {
            widths: ["*"],
 ...
 *
 * O cast para Content acontece somente aqui porque a definicao
 * de @types/pdfmake usada pelo projeto nao representa corretamente
 * width + stack em uma coluna.
 */
function coluna(
    pares: [string, string][],
): Content {
    const colunaPdf: ColunaPdf = {
        width: "*",
        stack: pares.map(
            ([rotulo, valor]) =>
                ({
    ...
    return colunaPdf as unknown as Content;
}
...
function bloco(
    ...
    return blocoPdf as unknown as Content;
}
...
const assinatura = (
    ...
    return assinaturaPdf as unknown as Content;
};
...

                    {
                        ...
                        table: {
                            ...
                            body: [
                                ...
                                [
                                    linhaTotal(
                                        `BDI / margem (${percentual.toString()}%)`,
                                    ),
